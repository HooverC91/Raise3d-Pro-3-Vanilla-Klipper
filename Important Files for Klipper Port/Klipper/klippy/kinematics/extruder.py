...
class PrinterExtruder:
    ...
    def get_name(self):
        return self.name
    def report_nozzle_diameter_info(self):
        self.nprintf("extruder N%d nozzle_diameter = %.2f"%(self.extruder_num,self.nozzle_diameter))
        self.nprintf("extruder N%d filament_diameter = %.2f"%(self.extruder_num,self.filament_area))
        self.nprintf("extruder N%d max_extrude_ratio = %.2f"%(self.extruder_num,self.max_extrude_ratio))
        self.nprintf("extruder N%d max_e_velocity = %.2f max_e_accel = %.2f"%(self.extruder_num,self.max_e_velocity,self.max_e_accel))
    def set_nozzle_diameter(self, diameter):
        self.nozzle_diameter = diameter
        self.filament_area = math.pi * (self.filament_diameter * .5)**2
        def_max_cross_section = 4. * self.nozzle_diameter**2
        def_max_extrude_ratio = def_max_cross_section / self.filament_area
        max_cross_section = self.max_extrude_ratio * self.filament_area
        self.max_extrude_ratio = max_cross_section / self.filament_area
        logging.info("Extruder max_extrude_ratio=%.6f", self.max_extrude_ratio)
        toolhead = self.printer.lookup_object('toolhead')
        max_velocity, max_accel = toolhead.get_max_velocity()
        ...
        #response latest max_e_velocity and max_e_accel
        self.report_nozzle_diameter_info()
    def save_nozzle_diameter(self,diameter):
        section = self.name
        self.printer._save_config(section, "nozzle_diameter", diameter)
        gcode = self.printer.lookup_object('gcode')
        gcode.respond_raw("%s set diameter = %.2f"%(self.name,diameter))
        pass
    def get_heater(self):
        return self.heater
    def get_trapq(self):
