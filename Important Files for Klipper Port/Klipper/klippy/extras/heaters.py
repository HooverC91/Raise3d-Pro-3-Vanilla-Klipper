...
class Heater:
    def __init__(self, config, sensor):
        ...
        if self.name == "extruder" or self.name == "extruder1":
            pwm_cycle_time = 0.004
            self.mcu_pwm.setup_cycle_time(pwm_cycle_time,True)
        else:
    ...
    def set_control(self, control):
        ...
        return old_control
    def pause(self):
        if self.is_heater_blocking == True:
            self.is_heater_blocking = False
...
class ControlPID:
    ...
    def temperature_update(self, read_time, temp, target_temp):
        if target_temp == 0 and self.last_target_temp > 0:
            self.last_target_temp = target_temp = 0
            self.prev_temp = temp
            ...
            self.heater.set_pwm(read_time, 0)
            return
        elif target_temp == 0 and self.last_target_temp == 0:
            self.prev_temp = temp
            self.prev_temp_time = read_time
            self.prev_temp_deriv = 0.
            self.prev_temp_integ = 0.
...
class PrinterHeaters:
    ...
    def _gcode_pause(self,msg):
        gcode = self.printer.lookup_object('gcode')
        for name in self.available_heaters:
            heater_to_pause = self.heaters[name]
            if heater_to_pause is not None:
                # try to pause heater
                heater_to_pause.pause()
        pass
    ...
    def _wait_for_temperature(self, heater):
        ...
        heater.is_heater_blocking = True
        while not self.printer.is_shutdown() and heater.check_busy(eventtime):
            print_time = toolhead.get_last_move_time()
            gcode.respond_raw(self._get_temp(eventtime))
            eventtime = reactor.pause(eventtime + 1.)
            if heater.is_heater_blocking == False:
                gcode.respond_raw("ok heater - %s paused"%(heater.name))
                heater.set_temp(0)
                break
        heater.is_heater_blocking = False
