...
class VirtualSD:
    def __init__(self, config):
        ...
        for cmd in ['M20', 'M21', 'M23', 'M24', 'M25', 'M26', 'M27', 'M32', 'M39']:
        ...
        self.is_gcode_pause = False
        self.printer.register_event_handler("gcode:pause", self._gcode_pause)
    def _gcode_pause(self,msg):
        if msg == 'M25':
            if self.work_timer != None and self.current_file is not None:
                self.is_gcode_pause = True
        pass
    ...
    def work_handler(self, eventtime):
        ...
        while not self.must_pause_work:
            ...
            try:
                ...
                report_gcode_cmds = ["G92","G91","G90","M83","M82"] 
                for cmd in report_gcode_cmds:
                    if line.startswith(cmd):
                        self.gcode.respond_raw("Gcode command: %s"%(line))
            ...
            block_cmds = ["M104","M109","M140","M190","G29"]
            
            
            # self.gcode.respond_raw("print line:%s"%(line))
            if self.is_gcode_pause == True:
                self.is_gcode_pause = False
                #check out if line is not empty
                if line.strip() == "" :
                    self.gcode.respond_raw("Emtpy line !")
                if line.strip() != "" and line.split()[0] in block_cmds:
                    self.gcode.respond_raw("Temp cmd : %s"%(line,))
                    self.file_position = self.next_file_position - len(line) - 1
                    self.gcode.respond_raw("Block command: %s and retreat"%(line))
                else:
                    self.file_position = self.next_file_position
            else:
                self.file_position = self.next_file_position

            # if line.split()[0] not in block_cmds:
            #     self.file_position = self.next_file_position
            #     if self.is_gcode_pause == True :
            #         self.is_gcode_pause = False
            # else:
            ...
            #     else:
            #         self.file_position = self.next_file_position
            # self.gcode.respond_raw("last print line:%s"%(line))
            ...
            if not data_flag:
                ...
                if (math.fabs(self.print_progress - temp_print_progress) > 0.1) or (temp_print_progress >= 100.0):
                    ...
                    pos = tuple(p)
                    self.gcode.respond_raw("SD printing progress %d/%d %d Current E%.3f" % (self.file_position, self.file_size, self.printed_lineno, pos[3]))
