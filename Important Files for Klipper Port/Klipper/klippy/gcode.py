...
class GCodeDispatch:
    ...
    def __init__(self, printer):
        ...
        # solve mutil-thread run and msg transmit
        r,w = os.pipe()
        self.pipe_r = r
        self.pipe_w = w
        self.pipe_is_active = False
        self.pipe_handle = self.reactor.register_fd(self.pipe_r,self._process_pipe)
    #write to pipe from bg thread
    def write_to_pipe(self, msg):
        if self.pipe_is_active:
            try:
                os.write(self.pipe_w, msg)
            except:
                raise logging.exception("write to pipe error")
        pass
    #only for multi-thread transmit msg when mutex don't work
    def _process_pipe(self, eventtime):
        try:
            data = os.read(self.pipe_r, 4096)
            self.respond_raw(data)
        except os.error:
    ...
    def cmd_default(self, gcmd):
        ...
        if not self.is_printer_ready:
            ...
            #gcmd._respond_error('Klipper is not ready: cant run %s' % (cmd,))
            return
        if not cmd:
...
class GCodeIO:
    ...
    def _process_data(self, eventtime):
        ...
        if lines[0].strip() == 'M25' or lines[0].strip() == 'M33':
            self.printer.send_event("gcode:pause",lines[0].strip())
